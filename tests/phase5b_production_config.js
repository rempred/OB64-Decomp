...
function copyPhase5a() {
  ...
  for (const row of manifest.files) { const out = path.join(target, row.path); fs.mkdirSync(path.dirname(out), { recursive: true }); fs.copyFileSync(path.join(PRODUCT, row.path), out); }
...
function provenanceRejectedBoth(mutate, label) {
  ...
  rejected(run([path.join(ROOT, 'tools', 'verify_phase5b_production_config.js'), '--config-root', root]), `${label} production`);
}
try {
