typedef signed char s8;
typedef unsigned char u8;
typedef signed short s16;
typedef unsigned short u16;
typedef signed int s32;
typedef unsigned int u32;
typedef signed long long s64;
typedef unsigned long long u64;
typedef float f32;
typedef double f64;
typedef s32 M2C_UNK;
typedef s8 M2C_UNK8;
typedef s16 M2C_UNK16;
typedef s32 M2C_UNK32;
typedef s64 M2C_UNK64;
M2C_UNK func_001F0E64(void *, M2C_UNK);                         
f32 func_001FF7EC(s16, s16);                                    
f32 func_002016F4(s32, s32);                                    
M2C_UNK func_0020BD5C(M2C_UNK);                                 
s32 func_0020BFF8(void **);                                     
s32 func_0020C014(void **);                                     
s32 func_0020C0CC(void **);                                     
s32 func_0020C2C0(void **);                                     
s32 func_0020C32C(void **);                                     
void **func_0020C478(s32);                                      
M2C_UNK func_0021824C(s32 *, s32 *, s32 *, s32 *);              

void func_00218B58(void) {
    s32 sp10;
    s32 sp14;
    s32 sp18;
    s32 sp1C;
    M2C_UNK var_a1_89;
    s32 temp_a0_116;
    s32 temp_a0_22;
    s32 temp_s2_276;
    s32 temp_v1_328;
    s32 var_a0_119;
    s32 var_s1_131;
    s32 var_s2_24;
    s32 var_s3_136;
    s32 var_s3_221;
    s32 var_v1_72;
    u8 temp_v1_331;
    void **temp_s3_63;
    void **temp_v0_138;
    void **temp_v0_229;
    void **temp_v0_27;
    void **var_s0_270;
    void **var_s0_40;
    void *temp_a0_352;
    void *temp_a0_91;
    void *temp_v0_335;
    void *temp_v1_288;
    void *temp_v1_296;
    void *temp_v1_305;
    void *temp_v1_313;
    void *temp_v1_371;

    func_0021824C(&sp10, &sp14, &sp18, &sp1C);
    temp_a0_22 = (*(s32 *)((s8 *)(*(void **)0x801CE8BC) + (0x6048)));
    var_s2_24 = 0;
    if (temp_a0_22 != 0) {
        goto block_28;
    }
loop_2:
    temp_v0_27 = func_0020C478(var_s2_24);
    if (func_0020C2C0(temp_v0_27) != 0) {
        goto block_26;
    }
    if (func_0020C32C(temp_v0_27) != 0) {
        goto block_26;
    }
    var_s0_40 = temp_v0_27;
    if ((*(u16 *)((s8 *)(temp_v0_27) + (0x20))) != 0) {
        goto block_10;
    }
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 1) {
        goto block_7;
    }
    var_s0_40 = temp_v0_27;
    if (func_0020C014(temp_v0_27) != 0) {
        goto block_10;
    }
block_7:
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 2) {
        goto block_26;
    }
    if (func_0020BFF8(temp_v0_27) == 0) {
        goto block_26;
    }
    var_s0_40 = temp_v0_27;
block_10:
    temp_s3_63 = var_s0_40 + 0xC;
loop_11:
    if (*var_s0_40 != 0) {
        goto block_13;
    }
    var_s0_40 += 4;
    goto block_25;
block_13:
    if (func_0020C014(temp_v0_27) == 0) {
        goto block_16;
    }
    var_v1_72 = sp10;
    if ((u32) (var_v1_72 - 1) < 2U) {
        goto block_22;
    }
block_16:
    if (func_0020BFF8(temp_v0_27) != 0) {
        goto block_18;
    }
    var_s0_40 += 4;
    goto block_25;
block_18:
    var_v1_72 = sp14;
    if ((u32) (var_v1_72 - 1) < 2U) {
        goto block_21;
    }
    var_s0_40 += 4;
    goto block_25;
block_21:
block_22:
    var_a1_89 = 5;
    temp_a0_91 = *var_s0_40 + 0x44;
    if (var_v1_72 != 1) {
        goto block_24;
    }
    var_a1_89 = 0xB;
block_24:
    var_s0_40 += 4;
    func_001F0E64(temp_a0_91, var_a1_89);
block_25:
    if ((u32) var_s0_40 < (u32) temp_s3_63) {
        goto loop_11;
    }
block_26:
    var_s2_24 += 1;
    if (var_s2_24 >= 0x14) {
        goto block_82;
    }
    goto loop_2;
block_28:
    if (sp18 == 0) {
        goto block_35;
    }
    if (*(u8 *)0x801CEAAB == 0xFF) {
        goto block_52;
    }
    temp_a0_116 = (temp_a0_22 - 0x10) * 4;
    var_a0_119 = temp_a0_116 & ((s32) ~temp_a0_116 >> 0x1F);
    if (var_a0_119 < 0x100) {
        goto block_32;
    }
    var_a0_119 = 0xFF;
    goto block_34;
block_32:
block_34:
    *(void **)0x801CEAAB = (u8) var_a0_119;
    goto block_53;
block_35:
    var_s1_131 = 1;
    if (sp10 != 0) {
        goto block_37;
    }
    if (sp14 == 0) {
        goto block_52;
    }
block_37:
    var_s3_136 = 0;
loop_38:
    temp_v0_138 = func_0020C478(var_s3_136);
    if (func_0020C2C0(temp_v0_138) != 0) {
        goto block_50;
    }
    if (func_0020C32C(temp_v0_138) != 0) {
        goto block_50;
    }
    if ((*(u16 *)((s8 *)(temp_v0_138) + (0x20))) != 0) {
        goto block_45;
    }
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 1) {
        goto block_43;
    }
    if (func_0020C014(temp_v0_138) != 0) {
        goto block_45;
    }
block_43:
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 2) {
        goto block_50;
    }
    if (func_0020BFF8(temp_v0_138) == 0) {
        goto block_50;
    }
block_45:
    if (func_0020C014(temp_v0_138) == 0) {
        goto block_47;
    }
    if (((sp10 == 0) | (sp10 == 3)) != 0) {
        goto block_50;
    }
block_47:
    if (func_0020BFF8(temp_v0_138) == 0) {
        goto block_49;
    }
    if (((sp14 == 0) | (sp14 == 3)) != 0) {
        goto block_50;
    }
block_49:
    var_s1_131 &= 0 - ((u32) (((*(u16 *)((s8 *)((*(void **)((s8 *)(temp_v0_138) + (0)))) + (0x1C))) + 0x18F) & 0xFFFF) >= 0x2BBU);
block_50:
    var_s3_136 += 1;
    if (var_s3_136 < 0x14) {
        goto loop_38;
    }
    if (var_s1_131 == 0) {
        goto block_53;
    }
block_52:
    func_0020BD5C(0xA);
    return;
block_53:
    var_s3_221 = 0;
    if (sp10 != 0) {
        goto block_55;
    }
    if (sp14 == 0) {
        goto block_82;
    }
block_55:
loop_56:
    temp_v0_229 = func_0020C478(var_s3_221);
    if (func_0020C2C0(temp_v0_229) != 0) {
        goto block_81;
    }
    if (func_0020C32C(temp_v0_229) != 0) {
        goto block_81;
    }
    if (func_0020C0CC(temp_v0_229) != 0) {
        goto block_81;
    }
    if ((*(u16 *)((s8 *)(temp_v0_229) + (0x20))) != 0) {
        goto block_64;
    }
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 1) {
        goto block_62;
    }
    if (func_0020C014(temp_v0_229) != 0) {
        goto block_64;
    }
block_62:
    if ((*(u8 *)((s8 *)(*(void **)0x801CE8BC) + (0x6087))) != 2) {
        goto block_81;
    }
    if (func_0020BFF8(temp_v0_229) == 0) {
        goto block_81;
    }
block_64:
    var_s0_270 = temp_v0_229;
    temp_s2_276 = (s32) (func_002016F4((*(s32 *)((s8 *)(temp_v0_229) + (0x48))), (*(s32 *)((s8 *)(temp_v0_229) + (0x4C)))) / 2.0f);
loop_65:
    if ((*(void **)((s8 *)(var_s0_270) + (0))) == 0) {
        goto block_80;
    }
    if (func_0020C014(temp_v0_229) == 0) {
        goto block_71;
    }
    if (sp10 != 1) {
        goto block_69;
    }
    temp_v1_288 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(u16 *)((s8 *)(temp_v1_288) + (0x1C))) = (u16) ((*(u16 *)((s8 *)(temp_v1_288) + (0x1C))) + temp_s2_276);
block_69:
    if (sp10 != 2) {
        goto block_79;
    }
    temp_v1_296 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(u16 *)((s8 *)(temp_v1_296) + (0x1C))) = (u16) ((*(u16 *)((s8 *)(temp_v1_296) + (0x1C))) - temp_s2_276);
    goto block_79;
block_71:
    if (sp14 != 1) {
        goto block_73;
    }
    temp_v1_305 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(u16 *)((s8 *)(temp_v1_305) + (0x1C))) = (u16) ((*(u16 *)((s8 *)(temp_v1_305) + (0x1C))) - temp_s2_276);
block_73:
    if (sp14 != 2) {
        goto block_75;
    }
    temp_v1_313 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(u16 *)((s8 *)(temp_v1_313) + (0x1C))) = (u16) ((*(u16 *)((s8 *)(temp_v1_313) + (0x1C))) + temp_s2_276);
block_75:
    if (sp18 == 0) {
        goto block_79;
    }
    if (sp1C != 0) {
        goto block_79;
    }
    if (sp14 == 0) {
        goto block_79;
    }
    temp_v1_328 = (*(u8 *)((s8 *)(temp_v0_229) + (0xAB))) - 4;
    temp_v1_331 = temp_v1_328 & ((s32) ~temp_v1_328 >> 0x1F);
    (*(u8 *)((s8 *)(temp_v0_229) + (0xAC))) = temp_v1_331;
    (*(u8 *)((s8 *)(temp_v0_229) + (0xAB))) = temp_v1_331;
block_79:
    temp_v0_335 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(u16 *)((s8 *)((*(void **)((s8 *)(var_s0_270) + (0)))) + (0x1E))) = (u16) (s32) func_001FF7EC((s16) (*(u16 *)((s8 *)(temp_v0_335) + (0x1C))), (*(s16 *)((s8 *)(temp_v0_335) + (0x20))));
    (*(u16 *)((s8 *)((*(void **)((s8 *)(var_s0_270) + (0xC)))) + (0x1C))) = (u16) (*(u16 *)((s8 *)((*(void **)((s8 *)(var_s0_270) + (0)))) + (0x1C)));
    (*(u16 *)((s8 *)((*(void **)((s8 *)(var_s0_270) + (0xC)))) + (0x1E))) = (u16) (*(u16 *)((s8 *)((*(void **)((s8 *)(var_s0_270) + (0)))) + (0x1E)));
    temp_a0_352 = (*(void **)((s8 *)(var_s0_270) + (0)));
    (*(s32 *)((s8 *)(temp_a0_352) + (4))) = (s32) ((s16) (*(u16 *)((s8 *)(temp_a0_352) + (0x1C))) * 0x1E);
block_80:
    var_s0_270 += 4;
    if ((u32) var_s0_270 < (u32) (temp_v0_229 + 0xC)) {
        goto loop_65;
    }
block_81:
    var_s3_221 += 1;
    if (var_s3_221 < 0x14) {
        goto loop_56;
    }
block_82:
    temp_v1_371 = *(void **)0x801CE8BC;
    (*(s32 *)((s8 *)(temp_v1_371) + (0x6048))) = (s32) ((*(s32 *)((s8 *)(temp_v1_371) + (0x6048))) + 1);
    return;
}
