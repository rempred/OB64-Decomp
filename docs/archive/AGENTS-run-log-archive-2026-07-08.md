# AGENTS.md Run-Log Archive (migrated 2026-07-08)

Verbatim archive of the run-log and status-narrative sections that accreted in
`AGENTS.md` between 2026-06-20 and 2026-06-24, migrated out on 2026-07-08 to
restore `AGENTS.md` as a thin rulebook (mirroring the parent workspace's
2026-07-02 AGENTS migration). Nothing here was edited; supersessions recorded
inside the entries remain accurate.

Current state lives in `docs/PLATFORM.md`, `docs/NEXT_STEPS.md`,
`docs/DECOMP_LOG.md`, and `docs/FINAL_DATA_OWNERSHIP_REPORT_2026-06-24.md`;
per-split detail lives in `docs/dossiers/`.

Contents, in original AGENTS.md order:

1. "Assembly-Backed Code Rebuild" current-result chunk-status narrative
   (formerly AGENTS.md lines 206-327).
2. "Chunks 0-33" status narrative plus the split-pipeline heads-up (formerly
   lines 343-527). The durable parent-function-DB hazard rules from this block
   were extracted into AGENTS.md "Parent Function DB Hazards" before archiving.
3. The ~85 per-split run-log sections, "First Decomp Loop: Boot Entry" through
   "Boot Resource Loader Callback Register Split" (formerly lines 529-2914).
   These same milestones are also chronicled parent-side in the parent
   `docs/OB64Decomp-log.md`.

---

## Part 1: Assembly-backed rebuild chunk-status narrative (ex AGENTS.md lines 206-327)
- Tracked real-assembler original-MIPS chunks: **100 composites** (chunk 0 177 `boot/`;
  chunks 1–99 in `lib/`: 350, 216, 67, 376, 88, 78, 103, 87, 34, 35, 191, 74, 67, 94, 153, 95, 66, 95, 80, 175, 99, 99, 73, 63, 71, 96, 142, 97, 103, 122, 86, 198, 109, 120, 134, 164, 180, 232, 155, 159, 160, 171, 90, 17, 15, 17, 27, 9, 11, 13, 13, 13, 9, 9, 9, 7, 1, 5, 3, 3, 5, 5, 5, 7, 15, 8, 21, 33, 23, 19, 23, 21, 33, 13, 7, 7, 15, 4, 11, 5, 4, 4, 5, 9, 9, 11, 4, 9, 5, 3, 4, 4, 4, 4, 3, 4, 4, 5, 1) = **6,181 real-assembler
  source files; 0 generated fallback chunks — the ENTIRE configured code region `0x00001000..0x0063676C` is
  now fully source-owned (data-ownership loop COMPLETE; final consolidated report:
  `docs/FINAL_DATA_OWNERSHIP_REPORT_2026-06-24.md`)**.
  Chunks 0–99 (`0x00001000..0x0063676C`) are fully source-owned as
  named code/data parts (chunks 90-99 own the **Section C HUFF pool tail** [0x5A1000..0x63676C; 36
  parser-backed parts cut at word-aligned HUFF block starts (magic-12) + chunk seams, 0 code; LOOP-COMPLETE
  at the configured stop 0x63676C incl. the terminal partial chunk 99 0x631000..0x63676C; Section C =
  N64 JPEG/NJPG-style "HUFF" entropy pool, 29 blocks, 18-byte container header decoded incl.
  leadU32==blockSize-4 self-relative length, Huffman entropy stage decoded to 230,400-byte coefficient
  buffers per block, final image render pending, data-only-safe]; Section A 0x301000..0x4E3140 fully AUDIO; chunk 78 crosses the Section A/B
  ...
  **Section B tail + Section C start** [0x591000..0x5A1000; 5 parts: block 61 tail + block 62 = anim-family
  ...
  start**, 0 code; RUN-COMPLETE resolving the prior partial-interior-chunk fallback; Section C =
  N64 JPEG/NJPG-style "HUFF" entropy pool, 29 blocks first 0x5943D4/last 0x630BC4, Huffman entropy stage
  decoded, final image render pending; B/C boundary pinned 0x594280])
  ...
  continuation `data_00420438` into chunk 66) (chunks 52-61 = Section A slice 2 [0x341000..0x3E1000]: 0 code + 64 data
  [37 data + 27 zero_fill], DATA TERRITORY — same asset family, TYPE UNRESOLVED, conservative names;
  0 jr$ra/0 prologues/0 pointers at all 4 byte alignments; parent-tooling-dark; container layout
  decoded; outgoing continuation `data_003DE988` into chunk 62; chunks 48-51 = Section A slice 1 [survey natural unit Section A
  ...
  chunk 46: 0 code + 17 data [9 data + 8 zero_fill], DATA TERRITORY —
  non-code high-entropy graphics/texture asset data past the executable extent, continuing chunk 45's
  ...
  chunk 45: 0 code + 15 data [8 data + 7 zero_fill], DATA TERRITORY —
  the entire 64 KiB is non-code high-entropy graphics/texture asset data past the executable extent,
  continuing chunk 44's `data_002CBA58` tail (0 jr$ra/0 prologues/0 pointers), 8 zero-fill object
  separators; no code, no straddler;
  chunk 44: 0 code + 17 data [9 data + 8 zero_fill], DATA TERRITORY —
  the entire 64 KiB is non-code data PAST the executable extent `0x2B89B4`: uniform high-entropy
  graphics/texture asset data continuing chunk 43's tail, 0 jr$ra / 0 prologues / 0 pointers,
  segmented at 8 real zero-fill runs; no incoming straddler (data continuation), no code;
  chunk 43: 81 code + 1 straddler-tail + 8 data, MIXED — the
  mission-briefing/scenario-overview overlay CODE region `0x2B1000..0x2B89B8` then the
  evidenced code→data transition at `0x2B89B8` (last function func_002B88C8 epilogue) and a
  non-code F3DEX2 display-list/float-pool/texture DATA tail `0x2B89B8..0x2C1000`; this is the
  chunk that crosses the audited executable-MIPS extent end `0x2B89B4`; chunk ends in data, NO
  outgoing straddler;
  chunk 39: 135 code + 19 data + 1 straddler-tail, MIXED —
  ...
  fallback chunks remain; the data-ownership loop is COMPLETE. Final consolidated report:
  `docs/FINAL_DATA_OWNERSHIP_REPORT_2026-06-24.md`.

---

## Part 2: Chunks 0-33 status narrative + pipeline heads-up (ex AGENTS.md lines 343-527)
...
## Part 3: Per-split run log (ex AGENTS.md lines 529-2914)
