...
service gate, boot resource global handle release helper, and boot resource
global handle slot record prepare helper into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 65 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 65 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00007560_00011000.s`. The next target is the
   `0x7560` leaf prefix / `0x7568` prologue helper that scans the corrected
   `0x800E82C8` slot-record array and calls `0x80077F88`.
