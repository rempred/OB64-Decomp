...
probe small-record copy/flag helper into named tracked parts while preserving
the exact rebuild gate. The current setup gate also builds a
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 55 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 55 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00005D9C_00011000.s`. The next target is the
   `0x5D9C` record checksum/signature helper with secondary entries at
   `0x5E84` and `0x5F00`.
