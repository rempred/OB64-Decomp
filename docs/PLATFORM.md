...
display-list finalize/flip helper, and display-list sync/modes helper into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 30 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 30 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00004048_00011000.s`. The next boundary has
   overlapping parent candidates at `0x4048` and `0x4050`; keep them together
   until the relationship is documented.
