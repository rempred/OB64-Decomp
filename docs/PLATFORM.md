...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,810 tracked source files total across chunks 0–61; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 38 generated fallback chunks.
...
- Chunks 0–61 (`0x00001000..0x003E1000`) are fully source-owned as named
  code/data parts (5,810 tracked source files: 177 in `boot/` + 5,633 in `lib/`;
  ...
  current split frontier `0x003E1000` (chunk 62, still a generated fallback chunk — deeper in the non-code data tail).
  chunk 1
...
- Source mix: 62 tracked composite real-asm chunks made from 5,810 tracked source
  files, plus 38 generated fallback chunks. (Current totals: 62 composites / 5,810 files / 38 fallback.)
