...
bitstream descriptor encode helper, resource probe init helper, and resource
probe finalize wrapper into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 38 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 38 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00004C5C_00011000.s`. The next target is the
   `0x4C5C` prologue routine with multiple callers/callees, two unresolved
   calls, one `jalr`, and reads from `0x800A8254/0x800A8258`.
