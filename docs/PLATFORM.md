...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,842 tracked source files total across chunks 0–65; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 34 generated fallback chunks.
...
- Chunks 0–65 (`0x00001000..0x00421000`) are fully source-owned as named
  code/data parts (5,842 tracked source files: 177 in `boot/` + 5,665 in `lib/`;
  ...
  current split frontier `0x00421000` (chunk 66, still a generated fallback chunk — the deferred N64 audio sound-bank).
  chunk 1
...
- Source mix: 66 tracked composite real-asm chunks made from 5,842 tracked source
  files, plus 34 generated fallback chunks. (Current totals: 66 composites / 5,842 files / 34 fallback.)
