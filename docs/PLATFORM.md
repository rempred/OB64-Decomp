...
helper, boot state slot target peer record dispatch helper, and boot state slot
flagged dispatch/lookup helper into named tracked parts while preserving the
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 68 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 68 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00007768_00011000.s`. The next target is the
   `0x7768` prologue helper, which parent evidence reports as a 644-byte
   permanent helper with secondary entries at `0x77D4`, `0x789C`, and
   `0x7924`, unresolved call target `0x80093540`, ten-slot scans, and writes
   around `0x800E7Axx`/`0x800E82xx`.
