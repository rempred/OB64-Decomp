...
display-list counter-step helper into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 31 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 31 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_000040B0_00011000.s`. The next boundary is a
   separate 552-byte prologue helper at `0x40B0` with secondary entry `0x42C4`.
