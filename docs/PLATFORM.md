...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,624 tracked source files total across chunks 0–43; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 56 generated fallback chunks.
...
- Chunks 0–43 (`0x00001000..0x002C1000`) are fully source-owned as named
  code/data parts (5,624 tracked source files: 177 in `boot/` + 5,447 in `lib/`;
  ...
  current split frontier `0x002C1000` (chunk 44, still a generated fallback chunk — past the executable extent, data territory).
  chunk 1
...
- Source mix: 44 tracked composite real-asm chunks made from 5,624 tracked source
  files, plus 56 generated fallback chunks. (Current totals: 44 composites / 5,624 files / 56 fallback.)
