...
cluster, boot state global reset helper, and boot state slot callback dispatch
helper into named tracked parts while preserving the exact rebuild gate. The
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 61 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 61 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00006EE8_00011000.s`. The next target is the
   related `0x6EE8` leaf / `0x6EF0` prologue sibling called by `0x27A0` and
   `0x102FA8`.
