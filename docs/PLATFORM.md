...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,746 tracked source files total across chunks 0–51; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 48 generated fallback chunks.
...
- Chunks 0–51 (`0x00001000..0x00341000`) are fully source-owned as named
  code/data parts (5,746 tracked source files: 177 in `boot/` + 5,569 in `lib/`;
  ...
  current split frontier `0x00341000` (chunk 52, still a generated fallback chunk — deeper in the non-code data tail).
  chunk 1
...
- Source mix: 52 tracked composite real-asm chunks made from 5,746 tracked source
  files, plus 48 generated fallback chunks. (Current totals: 52 composites / 5,746 files / 48 fallback.)
