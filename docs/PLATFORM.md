...
- `assemble_original_mips.js` currently uses 31 tracked composite
  ...
  `0x001E1000..0x001F1000` 122 files =
  3,666 tracked source files total), plus 69 generated fallback chunks.
...
- Chunks 0–30 (`0x00001000..0x001F1000`) are fully source-owned as named
  code/data parts (3,666 tracked source files: 177 in `boot/` + 3,489 in `lib/`;
  ...
  current split frontier `0x001F1000` (chunk 31, still a generated fallback chunk).
  chunk 1
...
- Source mix: 31 tracked composite real-asm chunks made from 3,666 tracked source
  files, plus 69 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files from the
   current frontier in `docs/NEXT_STEPS.md`: chunk 31 at `0x001F1000` (ALL CODE),
   first continuing outgoing straddler `func_001F0F9C` as
