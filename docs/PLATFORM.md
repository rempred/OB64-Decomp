...
indexed-record check helper into named tracked parts while preserving the exact
rebuild gate. The current setup gate also builds a
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 50 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 50 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00005978_00011000.s`. The next target is the
