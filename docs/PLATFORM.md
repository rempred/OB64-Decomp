...
helper, and bitstream cursor helper cluster into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 34 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 34 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_000046F4_00011000.s`. The next boundary is a
   416-byte prologue routine at `0x46F4`; it uses the same bit cursor globals
   as the newly split `boot_bitstream_cursor_helpers.s`.
