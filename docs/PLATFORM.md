...
clear helper, and byte copy/fill aligned leaves into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 99 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 99 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_0000A510_00011000.s`. The next target is the
   parent-labeled `seed::lzss_decompress` helper at `0xA510..0xAF7C`, frame
   size `0x28`, RAM `0x8007A110`, with a secondary entry at `0xABE0`. Parent
   `docs/rom-layout.md` records the LZSS token format from this function, so
   keep the full parent range together and use that doc as a semantic lead.
