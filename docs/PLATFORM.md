...
helper, bitstream cursor helper cluster, and bitstream descriptor decode helper
into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 35 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 35 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_00004894_00011000.s`. The next target is the
   overlapping `0x4894` leaf / `0x48C8` prologue bitstream helper pair; keep it
   together until the trailing `0x4AB8..0x4AC4` return/padding shape is
   documented.
