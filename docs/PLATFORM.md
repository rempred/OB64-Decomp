...
display-list counter-step/counter-packet helpers, and resource window cache
update helper into named tracked parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 33 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 33 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_000043D4_00011000.s`. The next boundary is a
   separate 800-byte prologue routine at `0x43D4`, before the following
   `0x46F4` prologue boundary.
