...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 78 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 78 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_0000874C_00011000.s`. The next target is the
   overlapping `0x874C` leaf / `0x8754` prologue routine, active in all states,
   with high-confidence callees `0x228D0`, `0x210C0`, and `0x21DD4`.
