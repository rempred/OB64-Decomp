...
service gate, and boot resource global handle release helper into named tracked
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 64 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 64 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_0000722C_00011000.s`. The next target is the
   larger `0x722C` leaf / `0x7234` prologue helper with secondary entries at
   `0x735C` and `0x745C`.
