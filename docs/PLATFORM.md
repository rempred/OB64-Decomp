...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,534 tracked source files total across chunks 0–42; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 57 generated fallback chunks.
...
- Chunks 0–42 (`0x00001000..0x002B1000`) are fully source-owned as named
  code/data parts (5,534 tracked source files: 177 in `boot/` + 5,357 in `lib/`;
  ...
  current split frontier `0x002B1000` (chunk 43, still a generated fallback chunk).
  chunk 1
...
- Source mix: 43 tracked composite real-asm chunks made from 5,534 tracked source
  files, plus 57 generated fallback chunks. (Current totals: 43 composites / 5,534 files / 57 fallback.)
