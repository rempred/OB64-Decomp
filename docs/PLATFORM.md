...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,850 tracked source files total across chunks 0–66; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 33 generated fallback chunks.
...
- Chunks 0–66 (`0x00001000..0x00431000`) are fully source-owned as named
  code/data parts (5,850 tracked source files: 177 in `boot/` + 5,673 in `lib/`;
  ...
  current split frontier `0x00431000` (chunk 67, still a generated fallback chunk — the WaveTables sample-payload continuation).
  chunk 1
...
- Source mix: 67 tracked composite real-asm chunks made from 5,850 tracked source
  files, plus 33 generated fallback chunks. (Current totals: 67 composites / 5,850 files / 33 fallback.)
