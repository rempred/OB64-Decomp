...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,641 tracked source files total across chunks 0–44; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 55 generated fallback chunks.
...
- Chunks 0–44 (`0x00001000..0x002D1000`) are fully source-owned as named
  code/data parts (5,641 tracked source files: 177 in `boot/` + 5,464 in `lib/`;
  ...
  current split frontier `0x002D1000` (chunk 45, still a generated fallback chunk — deeper in the non-code data tail).
  chunk 1
...
- Source mix: 45 tracked composite real-asm chunks made from 5,641 tracked source
  files, plus 55 generated fallback chunks. (Current totals: 45 composites / 5,641 files / 55 fallback.)
