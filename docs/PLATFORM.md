...
display-list counter-step and counter packet emit helpers into named tracked
parts while
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 32 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 32 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_000042D8_00011000.s`. The next boundary is an
   overlapping pair with leaf entry `0x42D8` and prologue entry `0x42E0`; keep
   that pair together unless a later dossier proves a safer split.
