...
clear helper, byte copy/fill aligned leaves, and the parent-labeled LZSS
decompressor into named tracked parts while
preserving the exact rebuild gate. The
...
- `assemble_original_mips.js` currently uses 1 tracked composite
  real-assembler chunk (`0x00001000..0x00011000`) made from 100 tracked source
...
- Source mix: 1 tracked composite real-asm chunk made from 100 tracked source
  files, plus 99 generated fallback chunks.
...
2. Continue splitting original MIPS into cleaner function/data files, starting
   from `asm/original/rev0/code_0000AF7C_00011000.s`. The next target is the
   no-name `func_0000AF7C` helper, size `0x30` / 48 bytes, frame size `0x18`,
   RAM `0x8007AB7C`, with one high-confidence call to RAM `0x80093810`. Keep
   the adjacent `0xAFAC` and `0xB030` resource-loader-looking helpers in view,
   but do not assign final semantics without better evidence.
