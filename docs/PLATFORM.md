...
- `assemble_original_mips.js` currently uses 41 tracked composite
  ...
  5,656 tracked source files total across chunks 0–45; the per-chunk enumeration above is a HISTORICAL setup-milestone snapshot through chunk 33 — see DECOMP_LOG / the chunk list for current per-chunk counts), plus 54 generated fallback chunks.
...
- Chunks 0–45 (`0x00001000..0x002E1000`) are fully source-owned as named
  code/data parts (5,656 tracked source files: 177 in `boot/` + 5,479 in `lib/`;
  ...
  current split frontier `0x002E1000` (chunk 46, still a generated fallback chunk — deeper in the non-code data tail).
  chunk 1
...
- Source-owner rebuild: 3 tracked non-code owner files under
  `data/source-owners/rev0/` (44,029 bytes), 1,054 generated fallback files under
...
- Source mix: 46 tracked composite real-asm chunks made from 5,656 tracked source
  files, plus 54 generated fallback chunks. (Current totals: 46 composites / 5,656 files / 54 fallback.)
