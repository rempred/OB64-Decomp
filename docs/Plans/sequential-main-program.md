...
3. Bounded readability source experiment: accepted at `81340e1` through its one final normal verifier. All three targets remain PURE_C with exact full-ROM output at CURRENT `DB5D230370D66813C424F62AB2AC3AC9AACD0D83DC2D46097EB18952B5455CDF`. Boot conversion R1 attempted all three targets: reset and recursive clear are provisional exact PURE_C; materializer retains a two-byte final-register mismatch after four pure forms and is restored to its accepted hybrid. No complete-wave verifier ran. Fresh bounded reasoning must resolve that source blocker while preserving the complete three-target gate.
