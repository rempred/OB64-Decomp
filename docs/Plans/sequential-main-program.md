...
4. Combat remaining source waves: pending. Accepted discovery at parent `e445991` adds seven inclusions and five shared integrations. Parent `f940a28` propagates accepted r2 and closes the fixed-image gate at static scope. Selector consumer provenance remains unresolved. Its preparation and observer adapter passed Critical offline review; an exact run configuration, load readback when applicable, runtime readiness, and Joe's separate per-run approval remain prerequisites.
