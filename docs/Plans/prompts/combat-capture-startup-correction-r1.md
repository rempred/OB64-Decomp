...
human_gate: none
launch_id: COMBAT-CAPTURE-STARTUP-CORRECTION-20260907-01
workspace_claim: C:/Users/Joe/Projects/OgreBattlel64/OB64 Decomp/docs/Plans/task-logs/combat-capture-startup-correction-r1.claim.json
---

# Correct capture startup and preserve failed-start evidence offline

W7 is accepted at canonical main 469a1416918592749d61dc34e3e079796f7b673c and its worker has explicitly released all production writes.
Use that commit as the canonical source baseline. Its sole normal verifier passed with CURRENT 2626DA69447FDA51B53CC4B0F6D740B516E43D12A5CB346B09E775493929632C.
The frozen W7 report is docs/Plans/task-logs/combat-compositing-wave7-r1.md, SHA256 7CE8CE89D543759DB99DB10B1831165E686C12D24ABE1F0756AE7824C7B0CCFA.
The Director resolved the canonical and parent Git roots, confirmed all prior internal writers released, reauthenticated the exact parent bridge bytes below, and found the assigned claim/report/output paths absent.
This assignment now owns the stated Total Resolver production surfaces and the exact parent bridge path. No other production writer is active; W8 waits for this tooling handoff and required review.
...
Use Astra Medium in eligible worker /root/compilation_groups_design. Director /root, native task 01a07262-aeca-7341-ad10-2dba705ff988, local.
...
Recorded parent bridge SHA256: 1133A66C5C5743B8B40F1CC410A6EB8A64639CAC2E750B74F6DFBC48186EBF1F. Recorded deployed port-64656 script SHA256: 7F87A8EA02DE7DA566AC201C32BFEC4FCCC7FA114F91C44D6E88FCFCD8DEEAC7. Authenticate current inputs before relying on these historical bindings.

Required result: a reviewable generic correction for the zero-stack focusedwatch wire failure and provenance-preserving handling of startup failure. Preserve manual Stop with no capture duration, first-hit or event-budget limit.
The real Python serializer and JavaScript command parser must interoperate for zero and positive stack counts. Zero must cause no stack-memory reads; malformed, fractional, negative, trailing-junk and out-of-range counts must be rejected. Preserve unrelated positive-only command validation and existing focused/cutscene behavior. Do not hide the defect by changing Combat to an arbitrary positive count or weakening identity/protocol checks.
For future startup failures, preserve owned evidence and close interrupted consistently in both startup modes even when instrumentation rollback clears the normal started flag. Never drain a different owner or epoch, invent lost baseline bytes, attribute residue to a new session, or weaken the nonempty-queue gate.
...
Create the complete fresh claim atomically before writes, recording both accepted repository baselines, receiver, Director/native identity, host, launch and revision.
Own that claim, docs/Plans/task-logs/combat-capture-startup-correction-r1.md, ignored build/total-resolver/combat-capture-startup-correction-r1/ and the activated production surfaces above.
You are not alone; preserve unrelated parent/canonical work and all frozen records. Do not revert others' changes. No agents, staging, commits, push, branches or worktrees.
Return exact changed-input identities, verification results, limitations and the deployment/recovery boundary to /root through collaboration; never native app sends/readbacks. Release all writes at terminal handoff. Independent tooling review is required before production deployment/use; this adds no ordinary matching review or extra ROM build.
