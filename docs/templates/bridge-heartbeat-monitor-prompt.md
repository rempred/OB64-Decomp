...
1. If `type` is `run_complete`:
   - If `payload.reviewDoc` is present, read that review doc and inspect repo
   ...
   - If `payload.reviewDoc` is missing, fall back to the older two-step path:
     read `docs/templates/review-handoff-prompt-template.md`, tailor a review
     handoff prompt from `payload.runSlug`/frontier/repo state, paste it into the
     GUI agent, and mark the event handled.

2. If `type` is `review_complete`:
   - This is the compatibility path for older prompts that requested a separate
     review-handoff step.
