# Boot Resource Probe Large Record Copy/Flag Split

Date: 2026-06-21

## Scope

This dossier records a static source-layout split of the Rev 0 permanent
boot-code helper immediately after the indexed-record copy/flag helper:

| Source | ROM range | RAM range | Notes |
| --- | --- | --- | --- |
| `asm/original/rev0/boot/boot_resource_probe_large_record_copy_flag.s` | `0x00005C58..0x00005CFC` | `0x80075858..0x800758FC` | Overlapping `0x5C58` leaf / `0x5C60` prologue helper that copies the `0x4AE8`-byte record at shared-buffer offset `0x30B0` into caller scratch and marks the buffer dirty/valid byte. |
| `asm/original/rev0/code_00005CFC_00011000.s` | `0x00005CFC..0x00011000` | `0x800758FC..0x80080C00` | Current tracked remainder. |

The name is conservative. The static copy/flag shape is clear, but no runtime
trace or controlled mutation has verified final behavior or record semantics.

## Static Evidence

- Parent `../scripts/ob64_symbols_v2.json` reports `0x5C58` as a 164-byte leaf
  entry and `0x5C60` as an overlapping 156-byte prologue body with frame size
  `0x20`.
- Parent snapshot data places the exact pair at fixed RAM
  `0x80075858/0x80075860` in all seven named states and all 21 parent RAM
  snapshots.
- Direct exact caller evidence is `0x553C`.
- Parent `../scripts/ob64_callgraph_v2.json` also folds sibling calls from
  `0x4C5C` and `0x539C` into this family; local source inspection shows those
  direct calls target RAM `0x800758FC`, so the `0x5CFC/0x5D04` sibling remains
  the next split.
- High-confidence callees are `resource_alloc` (`0x1330`), `0x1A4F0` / RAM
  `0x8008A0F0`, and `0x23460` / RAM `0x80093060`.
- Parent callgraph data reports no unresolved RAM calls for this helper.
- Xref/access evidence reports reads/writes of shared global pointer
  `0x800A83B8` and a byte write to `0x800A83BC`.
- Static code shape: the `0x5C58` prefix loads `0x800A83B8`, then the `0x5C60`
  prologue body branches on that loaded value.
- If `0x800A83B8` is zero, it allocates `0x8000` bytes through
  `resource_alloc`, stores the pointer globally, and fills the span in
  `0x100`-byte chunks through `0x8008A0F0(offset, buffer + offset, 0x100, 0)`.
- It copies `0x4AE8` bytes from `0x800A83B8 + 0x30B0` into caller scratch
  through `0x80093060`.
- It stores byte `1` to `0x800A83BC` before returning.

## Boundaries

- The split starts at parent leaf boundary `0x00005C58`, immediately after
  `boot_resource_probe_indexed_record_copy_flag.s`.
- The file keeps `0x5C58` and `0x5C60` together because the leaf prefix loads
  the shared global pointer used by the prologue body's first branch.
- The routine ends after the `jr ra` delay slot at `0x5CF8`.
- The next parent boundary is `0x00005CFC`; parent data reports another
  overlapping leaf/prologue helper family at `0x5CFC/0x5D04` with the same
  shared-buffer ensure/fill pattern and a `0x10` copy from offset `0`.

## Verification

After the split:

- `node tests\binutils_smoke.js` passed.
- `node tools\assemble_original_mips.js` passed.
- Full `node tools\verify_setup.js` passed.
- The assembled report shows 1 tracked composite real-asm chunk made from 54
  tracked source files, plus 99 generated fallback chunks.
- The assembled code-region SHA256 remains
  `40D4E7875BA50F005788611C63CF9C42D9154339B36793556BF045C25B64B409`.
- The rebuilt full-ROM SHA256 remains
  `571E83396BC81E70DA4C0A20313D82DBD7DFE685F2C37418C8E27F927E2CC67A`.
