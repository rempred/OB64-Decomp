...
#define RENDER_FLAGS (FIELD(palette, 20, 4) | FIELD(cmt, 18, 2) | FIELD(maskt, 14, 4) | \
                    FIELD(shiftt, 10, 4) | FIELD(cms, 8, 2) | FIELD(masks, 4, 4) | FIELD(shifts, 0, 4))
...
#define TEXTURE(imageWord, loadWord, renderWord, imageWidth, line, xshift) { \
    ...
    PAIR((loadWord) | (((line) & 0x1FF) << 9), LOAD_FLAGS); \
    PAIR(0xE6000000, 0); \
    PAIR(COORD(x0, y0, xshift, 0xF4000000), COORD(x1, y1, xshift, 0x07000000)); \
    PAIR(0xE7000000, 0); \
    PAIR((renderWord) | (((line) & 0x1FF) << 9), RENDER_FLAGS); \
    PAIR(COORD(x0, y0, 2, 0xF2000000), COORD(x1, y1, 2, 0)); \
}
